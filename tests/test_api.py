...
def test_agent_tools_include_survey_and_search(tmp_path, monkeypatch):
    ...
    names = set(all_tools())
    assert {"ask_survey", "search_standards", "read_url", "list_assembly", "place_instance", "sweep_mechanism", "define_joint", "check_mesh", "check_mechanism", "lookup_formula", "solve_formula", "run_solvers", "list_features", "patch_feature", "generate_designs", "check_feasibility", "search_parts", "verify_against_image", "export_urdf", "export_dxf", "list_cad_refs", "resolve_cad_ref", "draft_from_image"} <= names
