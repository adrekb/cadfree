...
FORMULAS: list[dict[str, Any]] = [
    ...
    _f(
        id="tip_speed",
        domain="turbo",
        title="Impeller tip / peripheral speed",
        latex=r"U = \omega r = 2\pi n r / 60",
        expr="(2 * pi * n_rpm / 60) * r",
        output="U",
        unit="m/s",
        variables={
            "n_rpm": _var(r"n", "1/min", "shaft speed"),
            "r": _var(r"r", "m", "radius (usually r_2)"),
        },
        disclaimer="Kinematics only. Not a material limit, not a burst test.",
        tags=("turbo", "impeller", "rotating"),
        source="Dixon / Stepanoff kinematics",
    ),
    _f(
        id="flow_cm",
        domain="turbo",
        title="Meridional velocity through an annular passage",
        latex=r"C_m = Q / (2\pi r b \psi)",
        expr="Q / (2 * pi * r * b * psi)",
        output="Cm",
        unit="m/s",
        variables={
            "Q": _var(r"Q", "m^3/s", "volume flow"),
            "r": _var(r"r", "m", "station radius"),
            "b": _var(r"b", "m", "passage height"),
            "psi": _var(r"\psi", "1", "area blockage", default=0.9),
        },
        disclaimer="Annulus continuity with a named blockage. Not a CFD passage average.",
        tags=("turbo", "impeller"),
        source="Dixon meanline",
    ),
    _f(
        id="euler_head",
        domain="turbo",
        title="Euler turbomachine head (no inlet swirl)",
        latex=r"H = U_2 C_{u2} / g",
        expr="U2 * Cu2 / g",
        output="H",
        unit="m",
        variables={
            "U2": _var(r"U_2", "m/s", "outlet tip speed"),
            "Cu2": _var(r"C_{u2}", "m/s", "outlet absolute whirl"),
            "g": _var(r"g", "m/s^2", "gravity", default=G),
        },
        disclaimer="Incompressible Euler with Cu1=0. Not a measured pump curve, not CFD.",
        tags=("turbo", "impeller", "head"),
        source="Euler / Dixon",
    ),
    _f(
        id="wiesner_slip",
        domain="turbo",
        title="Wiesner slip factor",
        latex=r"\sigma = 1 - \sqrt{\sin\beta_2}/z^{0.7}",
        expr="1 - sqrt(sin(beta2)) / (z ** 0.7)",
        output="sigma",
        unit="1",
        variables={
            "beta2": _var(r"\beta_2", "rad", "outlet blade angle from tangential"),
            "z": _var(r"z", "1", "blade count"),
        },
        disclaimer="Wiesner 1967 empirical slip. β2 from tangential. Not a CFD deviation.",
        tags=("turbo", "impeller"),
        source="Wiesner 1967",
    ),
    _f(
        id="specific_speed",
        domain="turbo",
        title="Dimensionless specific speed",
        latex=r"n_s = \omega \sqrt{Q} / (g H)^{3/4}",
        expr="(2 * pi * n_rpm / 60) * sqrt(Q) / ((g * H) ** 0.75)",
        output="ns",
        unit="1",
        variables={
            "n_rpm": _var(r"n", "1/min", "shaft speed"),
        ...
        disclaimer="SI omega-Ns. Not US gpm/ft Ns. Not a Cordier-line lookup.",
        tags=("turbo", "impeller"),
        source="Dixon dimensionless Ns",
    ),
    _f(
        id="affinity_Q",
        domain="turbo",
        title="Affinity law for flow",
        latex=r"Q_2 / Q_1 = n_2 / n_1",
        expr="Q1 * (n2 / n1)",
        output="Q2",
        unit="m^3/s",
        variables={
            "Q1": _var(r"Q_1", "m^3/s", "flow at speed n1"),
            "n1": _var(r"n_1", "1/min", "reference speed"),
            "n2": _var(r"n_2", "1/min", "new speed"),
        },
        disclaimer="Same machine, incompressible, similar velocity triangles. Not a new impeller.",
        tags=("turbo", "affinity"),
    ...
    _f(
        id="affinity_H",
        domain="turbo",
        title="Affinity law for head",
        latex=r"H_2 / H_1 = (n_2 / n_1)^2",
        expr="H1 * ((n2 / n1) ** 2)",
        output="H2",
        unit="m",
        variables={
            "H1": _var(r"H_1", "m", "head at speed n1"),
            "n1": _var(r"n_1", "1/min", "reference speed"),
            "n2": _var(r"n_2", "1/min", "new speed"),
        },
        disclaimer="Same machine, incompressible. Diameter affinity is not in this line.",
        tags=("turbo", "affinity"),
        source="Pump affinity laws",
    ),
    _f(
        id="npsh_available",
        domain="turbo",
        title="Available NPSH at the eye",
        latex=r"\mathrm{NPSH}_a = (p_{\mathrm{in}} - p_v)/(\rho g) + C_m^2/(2g)",
        expr="(p_inlet - p_vapor) / (rho * g) + (Cm ** 2) / (2 * g)",
        output="NPSHa",
        unit="m",
        variables={
            "p_inlet": _var(r"p_{\mathrm{in}}", "Pa", "inlet static pressure"),
            "p_vapor": _var(r"p_v", "Pa", "vapour pressure"),
            "rho": _var(r"\rho", "kg/m^3", "liquid density"),
            "Cm": _var(r"C_m", "m/s", "eye meridional speed"),
            "g": _var(r"g", "m/s^2", "gravity", default=G),
        },
        disclaimer="Incompressible NPSHa. NPSHr is a catalog/survey number — never invented here.",
        tags=("turbo", "cavitation", "npsh"),
        source="Hydraulic Institute NPSH definition",
    ),
    _f(
        id="disc_hoop",
        domain="turbo",
        title="Thin rotating-ring hoop stress",
        latex=r"\sigma_\theta = \rho \omega^2 r^2",
        expr="rho_solid * ((2 * pi * n_rpm / 60) ** 2) * (r ** 2)",
        output="sigma",
        unit="Pa",
        variables={
            "rho_solid": _var(r"\rho", "kg/m^3", "solid density"),
            "n_rpm": _var(r"n", "1/min", "shaft speed"),
            "r": _var(r"r", "m", "rim radius"),
        },
        disclaimer="Thin ring σ=ρω²r². Not a 3-D disc, not burst containment, not CalculiX CENTRIF.",
        tags=("turbo", "rotating", "stress"),
        source="Timoshenko rotating-ring first order",
    ),
    _f(
        id="hydraulic_power",
        domain="turbo",
        title="Ideal hydraulic power",
        latex=r"P = \rho g Q H",
        expr="rho * g * Q * H",
        output="P",
        unit="W",
        variables={
            "rho": _var(r"\rho", "kg/m^3", "fluid density"),
            "Q": _var(r"Q", "m^3/s", "volume flow"),
            "H": _var(r"H", "m", "head"),
            "g": _var(r"g", "m/s^2", "gravity", default=G),
        },
        disclaimer="Fluid power at Euler/spec head. Not shaft power, not η from a test stand.",
        tags=("turbo", "impeller"),
        source="First-law hydraulic power",
    ),
]
...
PACKS: dict[str, dict[str, Any]] = {
    ...
    "turbo": {
        "id": "turbo",
        "title": "Meanline impeller — Euler head, slip, Ns, NPSHa, hoop. Not CFD, not burst.",
        "formulas": [
            "tip_speed",
            "flow_cm",
            "wiesner_slip",
        ...
        "solvers": ["analytical", "fea", "radioss"],
    },
}
